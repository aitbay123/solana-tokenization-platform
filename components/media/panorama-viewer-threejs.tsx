...
interface PanoramaViewerThreeJSProps {
  src: string
  title?: string
  className?: string
}

export function PanoramaViewerThreeJS({ src, title, className }: PanoramaViewerThreeJSProps) {
  ...
  // Simple 3D panorama effect using CSS transforms
  useEffect(() => {
    ...
    const drawPanorama = () => {
      ...
      const pitchOffset = (pitch / 90) * (imageHeight * 0.1)
      
      // Draw the panorama with proper cylindrical projection
      ctx.drawImage(
        image,
        sourceX, pitchOffset, sourceWidth, imageHeight - Math.abs(pitchOffset) * 2,
        0, 0, width, height
      )
    }
